...
fn map_transfers(param: String, blk: eth::Block) -> Result<Option<erc721::Transfers>, substreams::errors::Error> {

    let mapped_contract= Hex::decode(param.clone()).unwrap();

    let transfers: Vec<_> = blk
        .events::<abi::erc721::events::Transfer>(&[&mapped_contract])
        .map(|(transfer, log)| {
